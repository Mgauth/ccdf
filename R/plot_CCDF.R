  ...
  if (class(Z[,1])=="NULL"){
    ...
    if (is.factor(X[,1])){
      ...
      ggplot() + ggtitle(colnames(Y)) +
        ...
        geom_point(data = df_plot, aes(x = y, y = ccdf_x, color = as.factor(x)), shape=16, size = 1) +
        geom_point(data = df_plot, aes(x = y, y = ccdf_nox, color="black"), shape=2, size = 1) +
        scale_color_manual(name = "", 
                           labels = c("CDF", paste0("CCDF_X X=", unique(df_plot$x)), "CCDF_noX"),
                           values = c(viridis(n=(l_X+2))[-(l_X+2)],"gold"),
                           guide = guide_legend(override.aes = list(linetype = c("solid",rep("solid",l_X),"solid"),
      ...
      ggplot() + ggtitle(colnames(Y)) +
        geom_step(data = df_plot, aes(x = y, y = cdf, color = viridis(n=3)[1]), size = 1, linetype="solid") +
