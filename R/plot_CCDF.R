  ...
  if (class(Z)=="NULL"){
    ...
    if (is.factor(X[,1])){
      ...
      ggplot() + ggtitle(colnames(Y)) +
        geom_step(data = df_plot,aes(x = y, y = cdf, color = viridis(n=(l_X+1))[1]), size = 1, linetype="dotted") +
        geom_step(data = df_plot, aes(x = y, y = ccdf, color = as.factor(x)), size = 1) +
        scale_color_manual(name = "", labels = c("CDF", paste0("CCDF X=", unique(df_plot$x)), "CCDF2"),
                           values = c(viridis(n=(l_X+1))[1],viridis(n=(l_X+1))[-1]),
                           guide = guide_legend(override.aes = list(linetype = c("dotted",rep("solid",l_X))))) + xlab("y") +
        ylab("value") + theme_bw() + theme(plot.title = element_text(hjust = 0.5))
    }
    else{
      ggplot() + ggtitle(colnames(Y)) +
        geom_step(data = df_plot,aes(x = y, y = cdf, color = viridis(n=2)[1]), size = 1, linetype="dotted") +
    ...
    if (is.factor(X[,1])){
      l_X <- length(unique(X[,1]))
      ggplot() + ggtitle(colnames(Y)) +
        geom_step(data = df_plot, aes(x = y, y = cdf, color = viridis(n=(l_X+2))[1]), size = 1, linetype="dotted") +
        ...
        scale_color_manual(name = "", 
                           labels = c("CDF", paste0("CCDF_X X=", unique(df_plot$x)), "CCDF_noX"),
                           values = c(viridis(n=(l_X+2))[-(l_X+2)],"gold"),
                           guide = guide_legend(override.aes = list(linetype = c("dotted",rep("solid",l_X),"solid"),
                                                                    shape = c(NA,rep(16,l_X),2)))) +
        xlab("y") +
        ylab("value") + theme_bw() + theme(plot.title = element_text(hjust = 0.5))
      ...
      ggplot() + ggtitle(colnames(Y)) +
        geom_step(data = df_plot, aes(x = y, y = cdf, color = viridis(n=3)[1]), size = 1, linetype="dotted") +
